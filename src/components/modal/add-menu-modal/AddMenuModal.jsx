...
import { CalendarDays, Upload, Check, X } from 'lucide-react'
      ...
      <div className="bg-white rounded-lg w-full max-w-lg mx-auto shadow-2xl relative animate-in zoom-in-95 duration-300 overflow-hidden max-h-[600px] flex flex-col">
        ...
        <div className="relative bg-gradient-to-br from-primary via-primary/90 to-primary/80 p-4 text-white flex-shrink-0">
          ...
          <div className="text-center pt-1">
            <h2 className="text-xl font-semibold">Add Weekly Menu Item</h2>
            <p className="text-xs text-white/90 mt-1">Create a new menu item for the week</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto scrl-hide">
          <form onSubmit={handleSubmit(onSubmit)} className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <DatePicker
                  ...
                  className="w-full border border-gray-300 px-2 py-1 rounded-xs text-xs text-gray-500 font-normal outline-none appearance-none pr-8 cursor-pointer"
                  customInput={
                    <button
                      type="button"
                      className="flex items-center justify-between w-full px-3 py-1 border border-gray-300 rounded-sm text-xs font-medium text-gray-700 hover:bg-gray-50 transition-all duration-200 bg-white"
                    >
              ...
              <div className="grid grid-cols-4 gap-2">
                {mealTypeOptions.map((meal) => (
                  <button
                    ...
                    className={`relative px-2 py-1 rounded-xs text-xs font-medium transition-all duration-200 border cursor-pointer ${selectedMealTypes.includes(meal.value)
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {selectedMealTypes.map((type) => {
                    ...
                    return (
                      <span
                        key={type}
                        className={`px-2 py-1 rounded-xs text-xs font-normal bg-gradient-to-r ${meal.color} text-white`}
                      >
                        {meal.label}
                      </span>
                    )
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <div className="w-fit px-2 border border-gray-300 rounded-sm py-1.5 text-center cursor-pointer hover:bg-gray-50 hover:border-primary/50 transition-all group">
                  <input
                    type="file"
                    className="hidden"
        ...
        <div className="flex-shrink-0 p-4 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-white">
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => setAddMenuModal(false)}
              className="flex-1 border border-gray-300 text-gray-700 px-4 py-1 rounded-sm text-xs font-medium hover:bg-gray-50 transition-all duration-200 cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary text-white px-4 py-1 rounded-sm text-xs font-semibold transition-all duration-300 shadow-lg hover:shadow-xl cursor-pointer"
            >
              Create Menu Item
            </button>
          </div>
